#' ---
#' title: Simulation study to assess adjusted predictions in GEEs (Poisson distributed responses).
...
here::i_am("simulations/poisson_n25m10.R")
library(here)
...
save(all_results, file = here("simulations", "poissonresp_AR1margcor_n25m10.RData"))
...
#' ## Do the simulation!
#' Dataset 653, 815, 948 due to fitting GEE (at least on the session information given below)
formula_X <- ~ 1 + treatment*time + x1 + x2 
all_results <- foreach(k0 = (1:1000)[-c(653,815,948)]) %dopar% sim_fn(k0 = k0,
                                                     formula_X = formula_X,
                                                     fixed_effects = true_fixed_effects,
                                                     data = dat,
                                                     margcor = true_R0,
                                                     ...
                                                     test_timepoints = 6:10,
                                                     true_correlation_structure = true_correlation_structure)
save(all_results, file = here("simulations", "poissonresp_exchmargcor_n25m10.RData"))
...
#' ## Do the simulation!
#' Dataset 38, 299, 348 hangs due to fitting GEE (at least on the session information given below)
formula_X <- ~ 1 + treatment*time + x1 + x2 
all_results <- foreach(k0 = (1:1000)[-c(38,299,348)]) %dopar% sim_fn(k0 = k0,
                                                     formula_X = formula_X,
                                                     fixed_effects = true_fixed_effects,
                                                     data = dat,
                                                     margcor = true_R0,
                                                     ...
                                                     test_timepoints = 6:10,
                                                     true_correlation_structure = true_correlation_structure)
save(all_results, file = here("simulations", "poissonresp_toepmargcor_n25m10.RData"))
...
all_results <- foreach(k0 = (1:1000)) %dopar% sim_fn(k0 = k0,
                                                     ...
                                                     test_timepoints = 6:10,
                                                     true_correlation_structure = "ar1")
save(all_results, file = here("simulations", "poissonresp_unstrucmargcor_n25m10.RData"))
